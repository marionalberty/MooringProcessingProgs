...
if params.print==1
  if isfield(params,'figs_lev0')
    ...
    %if ss=='y'
    nfop=sprintf('%s_%s_%s_%4.4d_%d_adcp_uqc',params.moor_name,...
      s.meterType,s.serialNo,s.plannedMeterDepth,params.qcLevel);
...
title(sprintf('Meridional Velocity QC (not mooring motion corrected) %s_%s_%s',...
  params.moor_name,s.meterType,s.serialNo),'interpreter','None')
...
if params.print==1
  if params.print==1
    if isfield(params,'figs_lev0')
      %ss=input('print figures ? ','s');
      %if ss=='y'
      nfop=sprintf('%s_%s_%s_%4.4d_%d_adcp_uraw',params.moor_name,...
        s.meterType,s.serialNo,s.plannedMeterDepth,params.qcLevel);
      if isfield(params,'dirpng')
        nfoppng=sprintf('%s/%s.png',params.dirpng,nfop);
        set(gcf,'units','centimeters')
...
if params.print==1
  if params.print==1
    if isfield(params,'figs_lev0')
      %ss=input('print figures ? ','s');
      %if ss=='y'
      nfop=sprintf('%s_%s_%s_%4.4d_%d_adcp_vraw',params.moor_name,...
        s.meterType,s.serialNo,s.plannedMeterDepth,params.qcLevel);
      if isfield(params,'dirpng')
        nfoppng=sprintf('%s/%s.png',params.dirpng,nfop);
        set(gcf,'units','centimeters')
...
axis([s.time(1) s.time(end) 1 nbins])
set(gca,'tickdir','out')
ylabel('ADCP bin','fontsize',16)
title(sprintf('Echo Amplitude Beam 1: %s_%s_%s',params.moor_name,...
  s.meterType,s.serialNo),'interpreter','None')
subplot(412)
...
set(gca,'tickdir','out')
ylabel('ADCP bin','fontsize',16)
title(sprintf('Echo Amplitude Beam 2: %s_%s_%s',params.moor_name,...
  s.meterType,s.serialNo),'interpreter','None')
subplot(413)
...
set(gca,'tickdir','out')
ylabel('ADCP bin','fontsize',16)
title(sprintf('Echo Amplitude Beam 3: %s_%s_%s',params.moor_name,...
  s.meterType,s.serialNo),'interpreter','None')
subplot(414)
...
set(gca,'tickdir','out')
ylabel('ADCP bin','fontsize',16)
title(sprintf('Echo Amplitude Beam 4: %s_%s_%s',params.moor_name,...
  s.meterType,s.serialNo),'interpreter','None')
orient tall
...
if params.print==1
  if params.print==1
    if isfield(params,'figs_lev0')
      %ss=input('print figures ? ','s');
      %if ss=='y'
      nfop=sprintf('%s_%s_%s_%4.4d_%d_echoamp',params.moor_name,...
        s.meterType,s.serialNo,s.plannedMeterDepth,params.qcLevel);
      if isfield(params,'dirpng')
        nfoppng=sprintf('%s/%s.png',params.dirpng,nfop);
        set(gcf,'units','centimeters')
...
% 2) The percentage of good transformations (error velocity threshold not
%    exceeded);
...
ylabel('ADCP bin','fontsize',16)
title(sprintf(...
  'Percent Good Beam 1: good of 3-beam solutions with 1-beam rejected: %s_%s_%s',...
  params.moor_name,s.meterType,s.serialNo),'interpreter','None')
subplot(412)
...
ylabel('ADCP bin','fontsize',16)
title(sprintf(...
  'Percent Good Beam 2: Good transformation where ErVel not exceeded: %s_%s_%s',...
  params.moor_name,s.meterType,s.serialNo),'interpreter','None')
...
if params.print==1
  if params.print==1
    if isfield(params,'figs_lev0')
      %ss=input('print figures ? ','s');
      %if ss=='y'
      nfop=sprintf('%s_%s_%s_%4.4d_%d_percgood',params.moor_name,...
        s.meterType,s.serialNo,s.plannedMeterDepth,params.qcLevel);
      if isfield(params,'dirpng')
        nfoppng=sprintf('%s/%s.png',params.dirpng,nfop);
        set(gcf,'units','centimeters')
...
title(sprintf('ADCP Pitch: %s_%s_%s',params.moor_name,s.meterType,...
  s.serialNo),'interpreter','None')
...
ylabel('degrees')
title(sprintf('ADCP Pitch Standard deviation: %s_%s_%s',params.moor_name,...
  s.meterType,s.serialNo),'interpreter','None')
...
datetick('x',12)
ylabel('degrees')
title(sprintf('ADCP Roll: %s_%s_%s',params.moor_name,s.meterType,...
  s.serialNo),'interpreter','None')
subplot(414)
...
datetick('x',12)
ylabel('degrees')
title(sprintf('ADCP Roll Standard Deviation %s_%s_%s',params.moor_name,...
  s.meterType,s.serialNo),'interpreter','None')
orient tall
...
if params.print==1
  if params.print==1
    if isfield(params,'figs_lev0')
      %ss=input('print figures ? ','s');
      %if ss=='y'
      nfop=sprintf('%s_%s_%s_%4.4d_%d_pitchroll',params.moor_name,...
        s.meterType,s.serialNo,s.plannedMeterDepth,params.qcLevel);
      if isfield(params,'dirpng')
        nfoppng=sprintf('%s/%s.png',params.dirpng,nfop);
        set(gcf,'units','centimeters')
...
datetick('x',12)
ylabel('degrees')
title(sprintf('ADCP Heading: %s_%s_%s',params.moor_name,s.meterType,...
  s.serialNo),'interpreter','None')
...
datetick('x',12)
ylabel('degrees')
title(sprintf('ADCP Heading Standard Deviation %s_%s_%s',params.moor_name,...
  s.meterType,s.serialNo),'interpreter','None')
orient tall
...
if params.print==1
  if params.print==1
    if isfield(params,'figs_lev0')
      %ss=input('print figures ? ','s');
      %if ss=='y'
      nfop=sprintf('%s_%s_%s_%4.4d_%d_heading',params.moor_name,...
        s.meterType,s.serialNo,s.plannedMeterDepth,params.qcLevel);
      if isfield(params,'dirpng')
        nfoppng=sprintf('%s/%s.png',params.dirpng,nfop);
        set(gcf,'units','centimeters')
...
title(sprintf('ADCP Temperature: %s_%s_%s',params.moor_name,s.meterType,...
  s.serialNo),'interpreter','None')
...
if params.print==1
  if params.print==1
    if isfield(params,'figs_lev0')
      %ss=input('print figures ? ','s');
      %if ss=='y'
      nfop=sprintf('%s_%s_%s_%4.4d_%d_temperature',params.moor_name,...
        s.meterType,s.serialNo,s.plannedMeterDepth,params.qcLevel);
      if isfield(params,'dirpng')
        nfoppng=sprintf('%s/%s.png',params.dirpng,nfop);
        set(gcf,'units','centimeters')
...
if (sum(s.pressure)>0)
  ...
  if strcmp(params.moor_name,'Solomon_M1') ||...
    ...
    datetick('x',12)
    ylabel('Depth (m)')
    title(sprintf('ADCP Depth: %s_%s_%s',params.moor_name,s.meterType,...
      s.serialNo),'interpreter','None')
  else
    ...
    axis ij
    title(sprintf('ADCP Pressure: %s_%s_%s',params.moor_name,s.meterType,...
      s.serialNo),'interpreter','None')
  end
