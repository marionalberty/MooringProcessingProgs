...
% East
load([pathin 'StGeorgesEast/StGeorgesEast.mat'],'params')
mlat(2) = params.lat;
mlon(2) = params.lon;
mbot(2) = params.bottomDepth;

% West
load([pathin 'StGeorgesWest/StGeorgesWest.mat'],'params')
mlat(1) = params.lat;
mlon(1) = params.lon;
...
% Get line that describes the moorings
p_moor = polyfit(mlon,mlat,1);
...
save([driveName 'sadcp_archive/StGeorges_meanAsvXsv.mat'],'data')
...
title('St. Geoerge''s Channel SADCP Monthly Breakdown')
...
colorbar
colormap(redblue(25))
caxis([-1 1]*max(abs(davdz(:)))*0.75)
xlabel('Distance Along Cross-Section [km]')
ylabel('Depth [m]')
title('St. George Along Channel Vertical Shear [1/s]')
...
colorbar
colormap(redblue(25))
caxis([-1 1]*max(abs(dxvdz(:)))*0.75)
xlabel('Distance Along Cross-Section [km]')
ylabel('Depth [m]')
title('St. George Across Channel Vertical Shear [1/s]')
...
ylabel('Depth [m]')
title('St. George Mean Across Channel Vertical Shear [cm s^{-1} m^{-1}]')
