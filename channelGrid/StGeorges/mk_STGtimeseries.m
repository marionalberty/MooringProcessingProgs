  ...
  for i_surf = 1:2
    for i_bot = 1:2
      for i_xpass = 1:3
        ...
        %% Load mooring data
        % St George's East
        load([driveName dataPath 'StGeorgesEast/StGeorgesEast_' method...
          '_' bot '_' surf '.mat'])
        UV_E = UV;    SS_E = SS;      TT_E = TT;      params_E = params;
        
        % St George's West
        load([driveName dataPath 'StGeorgesWest/StGeorgesWest_' method ...
          '_' bot '_' surf '.mat'])
        UV_W = UV;    SS_W = SS;      TT_W = TT;      params_W = params;
...
%         i_s = find(sum(isnan(M_filt.psal),2) == 0);
        ...
        rho_E = sw_dens(s_E,t_E,z_E);
        rho_W = sw_dens(s_W,t_W,z_W);
        ...
        %% Put data in grid
        % West
        [ASV(1:numel(UV_W.z),i_dW,:),XSV(1:numel(UV_W.z),i_dW,:)] = ...
          uvrot(V_W,U_W,phi);
        
        % East
        [ASV(1:numel(UV_E.z),i_dE,:),XSV(1:numel(UV_E.z),i_dE,:)] = ...
          uvrot(V_E,U_E,phi);
        ...
        %% Save data
        f_out = [driveName dataPath params_E.channel '_' xpass 'Xsection_'...
          'from_' method '_' bot '_' surf '.mat'];
        save(f_out,'ASV','XSV','z','time','bathy','RHO','TEM','SAL','inst_xz')
