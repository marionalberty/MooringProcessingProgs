...
%% Do interpolation with geostrophy
for i_method = 1:2
  %% Set cases
  ...
  %% Load mooring data
  % Solomon M1
  load([driveName dataPath 'Solomon_M1/Solomon_M1_' method '.mat'])
  UV_1 = UV;    SS_1 = SS;      TT_1 = TT;    params_1 = params;
  
  % Solomon M2a and Solomon M2b
  load([driveName dataPath 'Solomon_M2a/Solomon_M2a_' method '.mat'])
  SS_2 = SS;      TT_2 = TT;
  load([driveName dataPath 'Solomon_M2b/Solomon_M2b_' method '.mat'])
  UV_2 = UV;    params_2 = params;
  
  % Solomon M3
  load([driveName dataPath 'Solomon_M3/Solomon_M3_' method '.mat'])
  ...
  %% Filer and subsample data density
  % Solomon M1
  ...
  i_u = find(sum(isnan(UV_1.U),2)/numel(UV_1.time) < 0.01);
  % Subsample
  ...
  % Extract full coverage profiles
  i_s = find(sum(isnan(M_filt.psal),2) == 0);
  i_u = find(sum(isnan(UV_2.U),2)/numel(UV_2.time) < 0.01);
  % Subsample
  ...
  % Extract full coverage profiles
  i_s = find(sum(isnan(M_filt.psal),2) == 0);
  i_u = find(sum(isnan(UV_3.U),2)/numel(UV_3.time) < 0.01);
  % Subsample
  ...
  % West Segment (M1 & M2)
  % drho/dx
  z_top = max([z_1(1) z_2(1)]);
  z_bot = min([z_1(end) z_2(end)]);
  i_zt = find(z == z_top);
  i_zb = find(z == z_bot);
  drho1 = rho_2(z_2 >= z_top & z_2 <= z_bot,:) - ...
    rho_1(z_1 >= z_top & z_1 <= z_bot,:);
  dx1 = (bathy.dist(i_d2) - bathy.dist(i_d1))*1000;
  % grf = g/rho_0 f
  grf1 = repmat(g/sw_f(mean(bathy.lat(i_d1:i_d2)))./mean(...
    [rho_2(z_2 >= z_top & z_2 <= z_bot,:); ...
    rho_1(z_1 >= z_top & z_1 <= z_bot,:)]),i_zb-i_zt+1,1);
  % dv
  dv1 = dz * grf1 .* drho1 ./ dx1;
  % Use near bottom velocities for reference
  % M1
  [asv,~] = uvrot(v_1,u_1,phi_12);
  asv11 = [cumsum([dv1; asv(zu_1 == z_bot+dz,:)],'reverse'); ...
    asv(zu_1 > z_bot+dz,:)];
  z11 = [z_top:dz:zu_1(end)]';
  % M2
  [asv,~] = uvrot(v_2,u_2,phi_12);
  asv12 = [cumsum([dv1; asv(zu_2 == z_bot+dz,:)],'reverse'); ...
    asv(zu_2 > z_bot+dz,:)];
  z12 = [z_top:dz:zu_2(end)]';
  
  % East Segment (M2 & M3)
  % drho/dx
  z_top = max([z_2(1) z_3(1)]);
  z_bot = min([z_2(end) z_3(end)]);
  i_zt = find(z == z_top);
  i_zb = find(z == z_bot);
  drho2 = rho_3(z_3 >= z_top & z_3 <= z_bot,:) - ...
    rho_2(z_2 >= z_top & z_2 <= z_bot,:);
  dx2 = (bathy.dist(i_d3) - bathy.dist(i_d2))*1000;
  % grf = g/rho_0 f
  grf2 = repmat(g/sw_f(mean(bathy.lat(i_d2:i_d3)))./mean(...
    [rho_3(z_3 >= z_top & z_3 <= z_bot,:); ...
    rho_2(z_2 >= z_top & z_2 <= z_bot,:)]),i_zb-i_zt+1,1);
  % dv
  dv2 = dz * grf2 .* drho2 ./ dx2;
  % Use near bottom velocities for reference
  % M2
  [asv,~] = uvrot(v_2,u_2,phi_23);
  asv22 = [cumsum([dv2; asv(zu_2 == z_bot+dz,:)],'reverse'); ...
    asv(zu_2 > z_bot+dz,:)];
  z22 = [z_top:dz:zu_2(end)]';
  % M3
  [asv,~] = uvrot(v_3,u_3,phi_23);
  asv23 = [cumsum([dv2; asv(zu_3 == z_bot+dz,:)],'reverse'); ...
    asv(zu_3 > z_bot+dz,:)];
  z23 = [z_top:dz:zu_3(end)]';
  
  
  clear u_* v_* asv grf* drho* dv* dx*
  ...
  %% Apply different surface, bottom and cross-channel interpolation schemes
  for i_surf = 1:2
    for i_bot = 1:2
      for i_xpass = 1:2
        % Initialize ASVs
        ...
        switch i_surf
          case 1
            ...
            ASV11(1:i11+1,:) = interp1(z(i11:i11+1),ASV11(i11:i11+1,:),...
              z(1:i11+1),'linear','extrap');
            ASV12(1:i12+1,:) = interp1(z(i12:i12+1),ASV12(i12:i12+1,:),...
              z(1:i12+1),'linear','extrap');
            ASV22(1:i22+1,:) = interp1(z(i22:i22+1),ASV22(i22:i22+1,:),...
              z(1:i22+1),'linear','extrap');
            ASV23(1:i23+1,:) = interp1(z(i23:i23+1),ASV23(i23:i23+1,:),...
              z(1:i23+1),'linear','extrap');
          case 2
            surf = 'slab2surf';
            % Apply slab criteria
        ...
        switch i_bot
          case 1
            bot = 'linear2bot';
            % Apply linear criteria
            ASV11(i11:n1+1,:) = interp1(z([i11 n1+1]),ASV11([i11 n1+1],:),...
              z(i11:n1+1));
            ASV12(i12:n2+1,:) = interp1(z([i12 n2+1]),ASV12([i12 n2+1],:),...
              z(i12:n2+1));
            ASV22(i22:n2+1,:) = interp1(z([i22 n2+1]),ASV22([i22 n2+1],:),...
              z(i22:n2+1));
            ASV23(i23:n3+1,:) = interp1(z([i23 n3+1]),ASV23([i23 n3+1],:),...
              z(i23:n3+1));
          case 2
            bot = 'slab2bot';
            % Apply slab criteria
        ...
        ASV = channelInterpSS_G(ASV,bathy.dist,[i_d1 i_d2-1 i_d2 i_d3],...
          xpass);
        ...
        params_1.xpassage_method = ['geostrophic_' xpass];
        params_1.interp_method = method(1:end-6);
        ...
        %% Save data
        f_out = [driveName dataPath params_1.channel '_geostrophic_' ...
          xpass 'Xsection_from_' method '_' bot '_' surf '.mat'];
        save(f_out,'ASV','z','time','bathy','z_sgth267','z_sgth275','RHO',...
          'TEM','SAL','inst_xz')
