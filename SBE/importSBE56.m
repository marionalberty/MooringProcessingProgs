...
function [time, temp] = importSBE56(csv_filename)
...
if ~isempty(strfind(tline, '"Date","Time","Temperature"'));    
    ...
    %orig [param, ~] = textscan(fid,'%s%s%f','Delimiter','","');
...
% % % % % % % whos date
% % % % % % % date{1}
% % % % % % % matDate = datenum(date,'YYYY-MM-DD');
% % % % % % % times = (matTime + matDate);

matTime = datenum(time,'HH:MM:SS');
matdate = datenum(date,'yyyy-mm-dd');

times = matTime-datenum('01-Jan-2014'); %% need to make this more universal
